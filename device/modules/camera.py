        ...
        with self.lock:
            if self.cap and self.camera_connected:
                ...
                if ret:
                    ...
                    if not success:
                        return FrameMessage(error=True, message="can't encode frame", image=None)
                    jpg_as_text = base64.b64encode(buffer).decode('utf-8')
                    return FrameMessage(error=False, message="get frame successfully", image=jpg_as_text)
                else:
                    self.camera_connected = False
                    return FrameMessage(error=True, message="can't get frame", image=None)
            else:
                return FrameMessage(error=True, message="camera not connected", image=None)
